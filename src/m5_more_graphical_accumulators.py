...
def draw_squares_from_circle(n, circle, window):
    ...
    for k in range(n):
        ...
        square = rg.Square(new_center, side_length)
...
def run_test_draw_circles_from_rectangle():
    ...
    title = title + ' 8 blue in row, 3 in column; then 4 green in row, then' \
                    '5 in column'
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    low_left = rectangle1.get_lower_left_corner()

    distance_x = low_left.x - center1.x
    distance_y = low_left.y - center1.y
    ...
    end_y = center2.y + distance_y
    end_x = center2.x + distance_x
    ...
    for _ in range(n):
        ...
        start_y = start_y + distance_y
        start = rg.Point(start_x, start_y)

        end_x = end_x + distance_x
        end_y = end_y + distance_y
        end = rg.Point(end_x, end_y)

        if line_color == rectangle2_color:
            line_color = rectangle1_color
        else:
            line_color = rectangle2_color
